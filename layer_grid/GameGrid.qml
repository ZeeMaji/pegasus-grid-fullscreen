...
FocusScope {
    ...
    GridView {
		...
		// Maji Change - Anchor lefts as well to center the grid.
        anchors.left: parent.left
        anchors.leftMargin: root.gridMarginLeft
		...
		// Maji Change - Increase maximum cells by 2 each.
        property real columnCount: {
            if (cellHeightRatio > 1.2) return 7;
            if (cellHeightRatio > 0.6) return 6;
            return 5;
        }
        ...
        highlight: Rectangle {
            ...
            scale: 1.03 // Maji Change - Change selected scale from 1.20 to 1.03 so grids don't go off screen
            z: 2
        }
        ...
        delegate: GameGridItem {
            ...
            onDoubleClicked: {
				...
				// Maji Change - Launch games when double clicking.
				root.launchRequested();
                //root.detailsRequested();
            }
			// Maji Change - Add favorites toggle from layer_gameinfo/PanelLeft.qml
            function toggleFav() {
                if (game)
            ...
            Keys.onPressed: {
				...
				// Maji Change -  Add favorites button (X on ninty style gamepads)
